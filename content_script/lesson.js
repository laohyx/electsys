...
*/
	...
	for(radio_index = 0; radio_index < radiogroup.length ; radio_index++){
		jQuery(radiogroup[radio_index]).click(function(){
			setTimeout(function(){jQuery("input[value=课程安排]").trigger("click");}, 600);
		})
...
function is_list_full(){
	...
	document.lid_index = 0;
	setTimeout("processLidQueue();",0);
...
{
	var args = document.lids[document.lid_index++];
	if(document.lid_index == document.lids.length + 1){
		return;
	}
	var lid = args[0];
	var type = args[1];
...
{
	//setTimeout("processLidQueue();",50);
	//判断是否有错误提示
	error_pattern = new RegExp("<span id=\"lblMessage\" style=\"display:inline-block;height:32px;width:285px;\">(.*?)</span>");
	error_match = error_pattern.exec(html);

	if(error_match != null)
	{
		error_message = error_match[1];
		if(error_message.indexOf("参数错误") > -1){
			document.lid_index--;
			console.log('参数错误');
			//遇到错误，延迟调用处理函数
			setTimeout("processLidQueue();",200);
			return;			
		}
		/*
		if(error_message == "对不起，你选择的通识课门数已达到上限，不能继续增加通识课！")
			error_message = "通识达上限";
			//在列表上添加是否空的提示
		...
		lessontr[0].children(0).appendChild(fullspan);
		*/
		return 0;
	}
